...
a = plt.scatter(vCrime['Banks closed'],vCrime['Raw value'],)
b = plt.scatter(unem['Banks closed'],unem['Raw value'])
c = plt.scatter(unin['Banks closed'],unin['Raw value'])
d = plt.scatter(std['Banks closed'],std['Raw value'])
e = plt.scatter(inact['Banks closed'],inact['Raw value'])
f = plt.scatter(obs['Banks closed'],obs['Raw value'])
plt.xlabel("Banks Closed")
plt.ylabel("Raw Value")
plt.legend((a,b,c,d,e,f), ('Violent crime rate','Unemployment','Uninsured','Sexually transmitted infections','Physical inactivity','Adult obesity'), loc='upper right', ncol=3, fontsize=5)
