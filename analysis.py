...
plt.savefig('alcohol-cardio-by-year.png')
plt.clf()
...
plt.savefig('maternal-diabetes-by-year.png')
plt.clf()

plt.scatter(maternal_normalized, diabetes_normalized)
plt.savefig('maternal-diabetes.png')
plt.clf()

sns.heatmap(df.corr())
plt.title("Heatmap of Causes of Deaths Over Years", fontsize=16)
plt.savefig('causes_of_deaths_heatmap.png', bbox_inches='tight')
plt.clf()
